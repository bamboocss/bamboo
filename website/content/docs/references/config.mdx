...
Cascade layers, `cva`, recipes and patterns are unaffected. A group's class name is always a hash, so `grouped` turns
off [`cx` merging](/docs/concepts/writing-styles#classname-concatenation) for the same reason `hash.className` does.
Note that [`@bamboocss/vite`](/docs/guides/source-transformation) declines element splitting under `grouped`, since that
