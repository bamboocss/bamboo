...
<Callout type="warning">
  Some constructs do not work under `grouped` yet, and fail by rendering the element with **no** styles rather than
  wrong ones — the class the runtime asks for has no rule behind it. Use `cssMode: 'atomic'` if your app relies on them:
...
[`cx` merging](/docs/concepts/writing-styles#classname-concatenation) for the same reason `hash.className` does. Note
that [`@bamboocss/vite`](/docs/guides/source-transformation) declines element splitting under `grouped`, since that
optimization builds a class from a prefix and a value.
