...
| `--strict-tokens`       | Require every style value to be a token, so a raw css value is written `[14px]` | [`config.strictTokens`](/docs/references/config#stricttokens) |
...
Import `styles.css` for everything, or any of the files above on its own — the per-layer, per-recipe and per-theme files
are all importable directly.
