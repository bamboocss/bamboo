...
| `--config, -c <path>`         | Path to Bamboo config file                                    | [`config`](/docs/references/config)                           |
...
| `--cpu-prof`            | Generate a `bamboo-{command}-{timestamp}.cpuprofile` file for profiling | [Debugging](/docs/guides/debugging)                               |
| `--logfile <file>`      | Outputs logs to a file                                                  | [Debugging](/docs/guides/debugging)                               |
...
| `--silent`            | Whether to suppress all output                                          | [`config.logLevel`](/docs/references/config#log-level) |
| `--clean`             | Whether to clean the output directory before emitting                   | [`config.clean`](/docs/references/config#clean)        |
| `--config, -c <path>` | Path to Bamboo config file                                              | [`config`](/docs/references/config.md)                 |
| `--watch, -w`         | Whether to watch for changes in the project                             | [`config.watch`](/docs/references/config#watch)        |
...
When using `--splitting`, Bamboo will generate the following structure:
...
pnpm bamboo studio --build

# Preview built studio
pnpm bamboo studio --preview

# Use custom port
pnpm bamboo studio --port 3000
```
...
| `--config, -c <path>` | Path to Bamboo config file                                              | [`config`](/docs/references/config.md)                    |
| `--cwd <path>`        | Current working directory                                               | [`config.cwd`](/docs/references/config#cwd)               |
| `--cpu-prof`          | Generate a `bamboo-{command}-{timestamp}.cpuprofile` file for profiling | [Debugging](/docs/guides/debugging#performance-profiling) |
| `--logfile <file>`    | Outputs logs to a file                                                  | [Debugging](/docs/guides/debugging)                       |
...
pnpm bamboo mcp --config ./bamboo.config.ts

# Specify working directory
pnpm bamboo mcp --cwd ./my-project
```
