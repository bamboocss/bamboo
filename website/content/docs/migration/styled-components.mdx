...
In Bamboo, you need to decide which syntax you want to use. Bamboo recommends using the object syntax, but provides a way
...
In Bamboo, we model interpolations using the variants API. This allows define style groups or recipes that can be applied
to components.
...
In Bamboo, you can use the `globalCss` key of the `bamboo.config.ts` file to define global styles. This will automatically
