...
In Bamboo, you need to decide which syntax you want to use. Bamboo recommends using the object syntax, but provides a
way to opt-in to tagged template literals.
...
```bash
bamboo init -p --syntax template-literal --jsx-framework react
```
...
In Bamboo, you will use the autogenerate code in the `styled-system` directory at the root of your project.

> Remember to initialize your project with the `--syntax template-literal` flag or update the bamboo.config.ts file.
...
In Bamboo, it's pretty much the same thing except that you can't do any interpolation in the media query styles due the
static nature of Bamboo.
...
In Bamboo, you can use the pseudo props API to define responsive styles.
