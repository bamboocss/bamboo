...
In Bamboo, the base styles object needs to added to the `base` key of a `cva`.
...
In Bamboo, both kinds of recipe emit a named class per variant, into the `recipes` cascade layer. What you choose is
where the recipe lives, and what that costs.

- **Colocated** with the component, using the `cva` function. Every variant it declares is generated, because the build
  cannot cheaply find which ones your code uses.
...
`className` is optional — without it the class is derived by hashing the config, which is stable but not readable.

- **In the config**, by defining the recipe in `bamboo.config.ts`. This approach only generates the classes and css for
  the variants that are used in the project.
...
Notice that in Bamboo, you don't need to use the `$` prefix to access the tokens. If you really want to use the `$`
...
In Bamboo, you define the global styles in the `bamboo.config.ts` using the `globalCss` function.
