...
Bamboo has no `styled` factory. Base styles go under the `base` key of a recipe, and both kinds of recipe emit a named
class per variant, into the `recipes` cascade layer. What you choose is where the recipe lives, and what that costs.
...
In Bamboo, you define the global styles in the `bamboo.config.ts` using the `global.css` function.

> The styles are injected automatically under the `base` cascade layer via PostCSS
