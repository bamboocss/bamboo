...
> **Disclaimer:** This isn't about comparing which one is best. Bamboo and Stitches are two different CSS-in-JS solutions
...
In Bamboo, the base styles object needs to added to the `base` key.
...
In Bamboo, you can decide if you want unique classNames for the recipe or you want atomic classNames.
...
- **Selector classes** by defining the recipe in the `bamboo.config.ts` file. This approach only generates the classes
