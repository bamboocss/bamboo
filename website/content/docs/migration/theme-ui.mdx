...
In Bamboo, multi-variant styles are called recipes. Both kinds of recipe emit a named class per variant into the
`recipes` cascade layer, so a consumer's `css()` in the `utilities` layer overrides them by cascade. What you choose is
...
In Bamboo, global styles are defined in the top-level `globalCss` property of the bamboo config — not under `theme`.
