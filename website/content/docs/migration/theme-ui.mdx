...
- Bamboo and Theme UI both support JSX style props.
...
Bamboo relies on `postcss` and converts CSS-in-JS styles to static CSS at build-time, leading to better performance in
larger applications.
...
In Bamboo, you don't need to wrap your application in a `ThemeProvider` component. Instead, you can pass the theme object
to the `bamboo.config.js` file.
...
Bamboo offers three similar ways to style components. The first approach is to use the `styled` element syntax and rename
`sx` to `css`
...
In Bamboo, multi-variant styles are referred to as recipes. Recipes are a collection of styles that can be applied to any
component.

There are two ways to define recipes in Bamboo. The first approach is to use the `cva` function which produces atomic
classnames.
...
The second approach is to define the recipe in the `theme.recipes` property of the bamboo config. This is referred to as
'Config recipes' in Bamboo and allows for sharing recipes across components and projects.
...
In Bamboo, color modes related values are defined as `semanticTokens` in the theme. Semantic tokens are tokens that
...
In Bamboo, global styles are defined in the `theme.global` property of the bamboo config.
...
In Bamboo, these are called "layout patterns", or "patterns" for short. Bamboo provides similar patterns that can be used
