...
Bamboo offers three similar ways to style components. The first approach is to use the `styled` element syntax and
rename `sx` to `css`
...
In Bamboo, multi-variant styles are referred to as recipes. Recipes are a collection of styles that can be applied to
any component.
...
In Bamboo, these are called "layout patterns", or "patterns" for short. Bamboo provides similar patterns that can be
used as a function or JSX element just like Theme UI.
