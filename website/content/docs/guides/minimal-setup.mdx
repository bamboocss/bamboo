...
Bamboo doesn't automatically know which tokens are valid for which CSS properties, so it is necessary to tell Bamboo that
...
> Note: You don't need to install `@bamboocss/preset-base` or `@bamboocss/preset-bamboo`. Bamboo will automatically resolve
> them for you.
