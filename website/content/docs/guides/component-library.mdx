...
3. Run the `bamboo init` command in there to generate a `bamboo.config.ts` file, don't forget to set the `jsxFramework` if
   needed
...
Set the `importMap` in your `bamboo.config.ts` to that same package name. This will inform Bamboo which imports belong to
the `styled-system`.
