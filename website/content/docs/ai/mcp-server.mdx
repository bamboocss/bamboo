...
The server automatically loads your `bamboo.config.ts` from the current working directory when started.
...
If your Bamboo config is not in the default location, specify it explicitly:
...
```json
{
  "mcpServers": {
    "bamboo": {
      "command": "npx",
      "args": ["bamboo", "mcp", "--config", "./path/to/bamboo.config.ts"]
    }
  }
}
```
...
3. Check that `npx bamboo mcp` runs without errors
...
3. Verify files contain Bamboo CSS usage (css(), styled(), etc.)
