...
To use the text styles, we need to update the `config` object in the `bamboo.config.ts` file.
...
function App() {
  return <p className={css({ textStyle: 'body' })}>This is a paragraph from Bamboo with the body text style.</p>
}
```
...
function App() {
  return (
    <div>
      ...
      <h2 className={css({ textStyle: 'heading' })}>Uses DEFAULT variant</h2>
    </div>
  )
}
```

Layer styles and animation styles nest the same way.
