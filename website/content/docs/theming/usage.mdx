...
A path that is not a string literal — `token.var(key)` — is invisible to the reachability analysis behind
[`pruneUnusedTokens`](/docs/references/config#pruneunusedtokens), so the variable it names may not be declared. Spell
the path out, or set `pruneUnusedTokens: false`.
