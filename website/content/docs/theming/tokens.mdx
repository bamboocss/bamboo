...
defaults to `true`, and the gap is usually wide — the `vite-ts` example in this repository ships 480 tokens in
`tokens/index.mjs` and declares 45 of them in `styles.css`. Set it to `false` to declare every token.
