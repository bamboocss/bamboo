...
The fix is to give the two styles different origins. A component meant to be overridden should use `cva`/`sva`, which
puts its styles in the `recipes` layer — a consumer's `css()` in `utilities` then wins by
[cascade layer](/docs/concepts/cascade-layers), in every build.

If you own both call sites, merge the style objects instead: `css(base, override)` resolves per property before any
class name exists.
