...
Recipe classes are never merged, and neither is a class whose property is not a utility bamboo registered. Enabling
[`hash`](/docs/references/config#hash) turns merging off entirely, since a hashed class carries no property to compare.
