...
Declaring the component's styles as a **config recipe** — in `theme.recipes` or `theme.slotRecipes` — puts them in the
`recipes` layer, so a consumer's `css()` in `utilities` wins by [cascade layer](/docs/concepts/cascade-layers) in every
build. An inline `cva()` does **not** do this: its output is atomic, exactly like `css()`, and lands in `utilities`
