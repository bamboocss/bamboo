...
Bamboo uses static analysis to scan your JavaScript and TypeScript files for style props and function calls, generating
atomic CSS on demand for the call sites you actually wrote. It runs as a CLI or as a PostCSS plugin.

You author styles as plain objects; the tokens, recipes and variants you reach for come from your own config as
generated types, so the editor autocompletes them and the compiler checks them.
