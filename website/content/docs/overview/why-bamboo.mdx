...
Bamboo is a build-time CSS-in-JS engine. You write style objects; it reads them out of your source, emits atomic CSS,
and — wherever the value is knowable — rewrites the call into the class string it would have returned. Nothing generates
styles in the browser or injects them into the `<head>`.
...
and no style props, so a production build can [fold the call away entirely](/docs/guides/source-transformation):
...
A style that varies does not put the call back. Both branches of a ternary are known, so each resolves at build time and
only the choice is left behind — and even a value the build cannot see lowers, because the class is the property's
prefix plus whatever the value holds, and the prefix is known now.

The payoff is not the CPU that saves per call. It is that a bundle where everything lowered stops importing
`styled-system/css` at all, and the engine drops out of your app. On one sandbox here, with
[`leafFallback: false`](/docs/references/config#leaffallback) asserting that runtime values are scalars, a module with a
dynamic style goes from 7,542 bytes gzipped to 1,077 — because the last reference to the engine is gone rather than
merely unused.
...
patterns and recipes. The API is smaller and several behaviours differ — `cx` joins rather than merges, recipes are
named rather than atomic, and slot recipes scope their variants — so if you are coming from Panda, read
[what changed](/docs/overview/faq#how-is-bamboo-css-different-from-panda-css) before you
...
Bamboo runs anywhere PostCSS does, and there is a [CLI](/docs/installation/cli) for anywhere it does not.
