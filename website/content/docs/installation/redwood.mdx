...
This guide shows you how to set up Bamboo CSS in a Redwood project using PostCSS.
...
Install bamboo and generate the `bamboo.config.ts` and `postcss.config.js` file.
...
Redwood uses a `config` folder for all of the configuration files. Move the `bamboo.config.ts` and `postcss.config.js`
files to the `config` folder.

```bash
mv bamboo.config.ts postcss.config.js config/
```
