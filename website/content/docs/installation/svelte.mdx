---
title: Using Svelte
description: Easily use Bamboo with Svelte with our dedicated integration.
---

This guide will show you how to set up Bamboo CSS in a Svelte project using PostCSS.
...
To install Bamboo and corresponding dependencies run the following commands:
...
This `"prepare"` script will run Bamboo CSS CLI codegen before each build. Read more about
...
Make sure that all of the paths of your Svelte components are included in the `include` section of the `bamboo.config.ts`
file.
...
To configure Svelte preprocess to use PostCSS and add Bamboo alias update the `svelte.config.js` file as follows:
