...
This guide will show you how to set up Bamboo CSS in a React Router project using PostCSS.
...
### Replace TailwindCSS with BambooCSS

Update the `vite.config.ts` file to use BambooCSS instead of TailwindCSS.
...
export default defineConfig({
  css: {
    postcss: {
      plugins: [bamboocss, autoprefixer],
    },
  },
