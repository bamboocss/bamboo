...
A consumer's `className` wins over the recipe's own styles, because `css()` output sits in the `utilities` layer and a
recipe's in `recipes`. See [Cascade Layers](/docs/concepts/cascade-layers).
