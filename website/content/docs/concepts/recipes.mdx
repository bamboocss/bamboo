...
`className` is authoring metadata and remains accepted for shared configs and extraction-only tooling:
...
| Can be imported by another module                    | ✅ yes                                | ❌ no — see below                                                    |
...
### An inline recipe cannot be shared across modules

This is the constraint that decides how shared styles are organised, so it is worth stating directly rather than reading
off the table.

An inline recipe's declaration is **erased** from the module that declares it — that is what lets the config leave the
bundle. The binding's value is then `undefined`, so any module that imports it receives nothing, and the build fails
rather than shipping that:

```tsx
// app/styles.ts
export const probeRecipe = cva({ base: { display: 'block' } })

// app/root.tsx
;<span className={probeRecipe({ on: true })} /> // probeRecipe — runtime-binding
```

The call site being fully literal does not help; nothing about the _selection_ is the problem. A recipe used in more
than one file has to be a config recipe, declared under `theme.extend.recipes` and imported from
`styled-system/recipes`. Inline recipes are for styles that stay in the file that declares them. | Can accept a runtime
variant selection under Vite | ✅ finite axes become a precompiled decision table | ✅ finite axes become a precompiled
decision table | | Can use a responsive object as the runtime selection | ❌ put responsiveness in the recipe's style
declarations | ❌ put responsiveness in the recipe's style declarations |

These rows describe the Vite compiler. Extraction-only CLI or PostCSS output retains the generated recipe runtime and
its named recipe representation.
