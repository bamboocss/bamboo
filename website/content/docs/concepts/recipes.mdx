...
@layer recipes {
  ...
  .cva_bMDqTx--visual_solid {
    background-color: #fed7d7;
    color: #fff;
  }

  .cva_bMDqTx--visual_outline {
    border-width: 1px;
    border-color: #fed7d7;
  }
  /* ... */
...
`cva_bMDqTx` above is derived by hashing the config, because an inline recipe has no name to take. Both the stylesheet
...
that matches the recipe-variant name pattern `<recipe-className>--<variant-name>`.
...
- Due to static nature of Bamboo, it's not possible to track the usage of the recipes in all cases. Here are some of use
  ...
  **When you use the recipe in a custom component that is not named as per the recipe name, Bamboo won't be able to
  track the usage of the recipe variants.**
...
at the start of the bamboo process when we evaluate your config.
