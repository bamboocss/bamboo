...
reach the CSS — because all of `config.recipes` is known when the config is evaluated.
[Atomic recipes](/docs/concepts/recipes#atomic-recipe-or-cva) (`cva`) are scattered through your source instead, so all
of their variants are emitted rather than scanning your app code repeatedly to find which are used.
