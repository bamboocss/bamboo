...
When a recipe is created, Bamboo will extract and generate CSS for every variant and compoundVariant `css` ahead of
time, as atomic classes.
...
and that means a [config recipe](#config-recipe). An inline `cva()` like the one above is atomic — its output is in
`utilities`, the same layer as the consumer's `css()` — so which one applies is decided by stylesheet order. Either
hoist the recipe into the config, or accept a style object and merge it:
`css(buttonStyle.raw(variantProps), props.css)`.
