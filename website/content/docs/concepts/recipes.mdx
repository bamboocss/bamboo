...
Atomic recipes are a way to create multi-variant styles with a type-safe runtime API, colocated with the component
rather than declared in your config.

> **The name is historical.** `cva` does not emit atomic classes. Its output is named from the config —
> `button--size_sm` — and lands in the `recipes` [cascade layer](/docs/concepts/cascade-layers), exactly like a recipe
> declared in `theme.recipes`. "Atomic" here means _defined inline_; the difference from a config recipe is where the
> name comes from, not what the CSS looks like.
...
evaluated. [Atomic recipes](/docs/concepts/recipes#atomic-recipe-or-cva) (`cva`) are scattered through your source
