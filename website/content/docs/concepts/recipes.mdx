...
When a recipe is created, Bamboo will extract and generate CSS for every variant and compoundVariant `css` ahead of time,
as atomic classes.
...
- Due to static nature of Bamboo, it's not possible to track the usage of the recipes in all cases. Here are some of use
  ...
  **When you use the recipe in a custom component that is not named as per the recipe name, Bamboo won't be able to track
