...
> Two patterns were removed because each was a second spelling of something the system already had. `box(styles)` had no
> styles of its own and was exactly `css(styles)`; `visuallyHidden()` was `css({ srOnly: true })`, wrapping a utility.
> Use the `css` form for both.
...
The HStack pattern lays its children out in a `row` and centers them vertically. It accepts `gap` and `justify` (an
alias for the css `justify-content` property).
...
The VStack pattern lays its children out in a `column` and centers them horizontally. It accepts `gap` and `justify` (an
alias for the css `justify-content` property).
