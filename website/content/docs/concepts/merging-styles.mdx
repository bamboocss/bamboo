...
`cx` also resolves conflicts between the classes you give it: two atomic classes for the same property under the same
conditions cannot both apply, so the last one passed wins rather than whichever the stylesheet ordered last.

```js
cx(css({ paddingX: '4' }), css({ paddingX: '2' })) // => 'px_2'
```

Recipe classes are never merged, which is why the example above keeps `button` and `button--size-small` intact. See
[Classname concatenation](/docs/concepts/writing-styles#classname-concatenation) for the full rules, including the case
where a hand-written class shaped like a utility class gets merged away and the builds where `cx` cannot merge at all.

The config recipe override above does not rely on that merge — `utilities` is declared after `recipes`, so it wins by
layer order whatever `cx` does with the class names.
...
> **Note:** For this to work, Bamboo requires that you set `jsxFramework` config option to `react`
