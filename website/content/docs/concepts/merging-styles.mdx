...
This is deliberate. `cx` cannot resolve conflicts in every build — under [`hash`](/docs/references/config#hash) a class
name is an opaque digest with no property to compare. A `cx` that merged where it could and joined where it could not
