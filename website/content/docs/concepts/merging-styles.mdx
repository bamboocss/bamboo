...
- **A component a consumer will override** belongs in the `recipes` layer. Write it with `cva`/`sva`, not bare `css()`,
  and the consumer's `css()` in `utilities` wins by layer — deterministically, in every build.
- **Two `css()` calls you own** are in the same layer. Merge the objects rather than the classes: `css(base, override)`
