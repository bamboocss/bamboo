...
If the enclosing slot is called something else, name it with `scopeRoot`:
...
Only slots rendered _inside_ the named one are reached — `trigger` above is a sibling of `positioner`, so a portal or a
sibling slot still needs its variant delivered by hand.

A recipe whose slots are all siblings has no ancestor to scope by. Those keep a variant class per slot, and every slot
stays callable.
...
The exception is a **portal**. A positioner rendered outside the root's subtree breaks DOM ancestry, so CSS cannot reach
it and the variant has to be delivered by hand. `slotsAffectedBy` says which slots a variant actually writes styles for,
so only those need threading:
...
For that case, a context provider is still the answer.
