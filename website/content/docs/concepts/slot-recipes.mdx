...
A recipe whose slots are siblings, with no slot named `root`, has no ancestor to scope by. Those keep a variant class
per slot, and every slot stays callable.
