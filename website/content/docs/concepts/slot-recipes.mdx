...
Bamboo emits the recipe into the `recipes.slots` layer: one constant class per slot, and each variant's rules `@scope`d
by the class the root slot carries for it.
...
@layer recipes.slots {
  ...
  @scope (.sva_gCdRhI__root--size_sm) to (.sva_gCdRhI__root) {
    .sva_gCdRhI__control {
      width: var(--sizes-8);
      height: var(--sizes-8);
    }
  ...
  /* ... and the same block for size_md */
}
```

The non-root slots carry no variant class of their own — see [Variant scoping](#variant-scoping) for why.
...
The generated css is registered under the `recipes.slots` [cascade layer](/docs/concepts/cascade-layers). Every slot
carries a constant `<recipe-className>__<slot-name>` class; the anchor slot additionally carries
`<recipe-className>__<slot-name>--<variant-name>_<variant-value>`, which scopes the other slots' variant rules — see
[Variant scoping](#variant-scoping).
