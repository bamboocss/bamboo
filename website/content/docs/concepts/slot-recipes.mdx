...
A **portal** keeps one context, and it is worth being exact about what for. Two different things can need the variant
delivered:

1. **A second anchor.** The portaled subtree is its own anchor, so name it in
   [`scopeRoots`](#components-that-span-a-portal) alongside the root — and then _deliver the variant to it_, because an
   anchor is callable and the consumer authors it as a sibling of the root. This is the one context a portaled component
   keeps; see [The second anchor still needs the variant](#the-second-anchor-still-needs-the-variant).
2. **A slot no anchor reaches.** Once both anchors are named this is usually empty. When it is not, `slotsAffectedBy`
   says which slots a variant actually writes styles for, so only those need threading:
   ...
   [`auditSlotScopes`](#catching-an-unreachable-slot-in-development) reports this case at runtime in development, since
   no build step can detect it.

A component with no portal has a single anchor — the element already receiving the props — so it needs no context at
all.

> **Note:** This is an advanced topic and you don't need to understand it to use slot recipes.

The one shape that still needs a context is a component that spans a portal, and it needs it for its _second anchor_
rather than for its slots. `createStyleContext`-style helpers — a provider per slot, wrapping every part — were the
answer when each slot carried its own variant class. Scoping removes that need: a slot's class is a constant, so it can
be read directly.
...
If you use React and do need a context for a portaled anchor, remember it makes the file a client component — add
