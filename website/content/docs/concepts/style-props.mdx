...
While you can get very far by using the `className` prop and function from Bamboo, style props provide a more ergonomic
way of expressing styles.

Bamboo will extract the style props through static analysis and generate the CSS at build time.
...
Using JSX style props is turned off by default in Bamboo, but you can opt-in to this feature by using the `jsxFramework`
property in the bamboo config.

> ⚠️ Bamboo will not extract style props from JSX elements if you don't set the `jsxFramework` property. This is to
> avoid unnecessary work for projects that don't use JSX.

### Choose Framework

JSX is a JavaScript syntax extension that allows you to write HTML-like code directly within your JavaScript code and is
supported by most popular frameworks. Bamboo supports JSX style props in React, Preact, Vue 3, Qwik and Solid.js.
...
Next, you need to run `bamboo codegen` to generate the JSX runtime for your framework.
...
<Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
  ...
  <Tab>
  ```bash
  npm bamboo codegen --clean
  ```
  </Tab>
  <Tab>
  ```bash
  yarn bamboo codegen --clean
  ```
  </Tab>
  <Tab>
  ```bash
  bun bamboo codegen --clean
  ```
  </Tab>
  {/* <!-- prettier-ignore-end --> */}
</Tabs>
...
<Callout type="warning">Due to the static nature of Bamboo, you can't rename properties at runtime.</Callout>
...
const CustomBox = (props: Props) => {
  ...
  return (
    <styled.div
      // ❌ Avoid: Bamboo can't know that you're mapping `boxPadding` to `padding`
      padding={boxPadding}
    />
  )
}
...
const App = () => {
  return (
    // In this case, you should keep the `padding` naming
    <CustomBox boxPadding="4" />
  )
}
```

The same principles apply to all style props and recipe variants.
...
Each pattern is a function that returns a class name, so it composes with any element.
...
const App = () => (
  <div className={stack({ gap: '4', align: 'flex-start' })}>
    <button>Button</button>
    <div className={circle({ size: '4', bg: 'red.300' })}>4</div>
  </div>
)
```
...
> For this to work correctly, set the `jsxFramework` to the framework you're using in your bamboo config.
...
Bamboo provides type definitions for all the style props that are supported by the JSX runtime.
...
Every pattern provided by Bamboo has a corresponding type that you can use to get type safety in your components.
