...
The base presets work the same way — here, dropping the `stack` pattern from `@bamboocss/preset-base`:
...
> ⚠️ `eject: true` is required in that second case: without it `@bamboocss/preset-base` is resolved anyway and puts
> `stack` back. See [which presets are included](/docs/customization/presets#which-bamboo-presets-will-be-included-).
