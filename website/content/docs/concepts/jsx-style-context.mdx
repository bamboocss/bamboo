...
The `createStyleContext` function can also be used with slot recipes defined in the `bamboo.config.ts` file.
...
> If you want to use a custom name, you can configure the recipe's `jsx` property in the `bamboo.config.ts` file.
