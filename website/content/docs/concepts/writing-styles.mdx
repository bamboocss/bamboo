...
- **A component you want reliably overridable** needs its styles in a lower cascade layer than the consumer's. Only a
  **config recipe** — one declared in `theme.recipes` or `theme.slotRecipes` — lands in `recipes`. An inline `cva()` is
  atomic, exactly like `css()`, and sits in `utilities` alongside the consumer.
