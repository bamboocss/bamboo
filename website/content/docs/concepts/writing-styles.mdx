...
- **A component a consumer will override** belongs in the `recipes` layer — write it with `cva`/`sva` rather than bare
  `css()`, and a consumer's `css()` in `utilities` wins by [cascade layer](/docs/concepts/cascade-layers) in every
  build.
- **Two `css()` calls you own** are in the same layer, so merge the objects rather than the classes: `css(a, b)`
  resolves per property before any class name exists.
