...
// one compact atom for padding-inline: 2
```
...
The `hash` option shortens extraction-only semantic class names and CSS variables. Vite uses
[`denseClassNames`](/docs/installation/vite#plugin-options) for compiler-owned atoms independently; this setting is
still useful when CSS variable names should also be hashed.
