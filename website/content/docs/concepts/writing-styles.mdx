...
visible in the name — with [`hash`](#hashing) it is not, and with
[`cssMode: 'grouped'`](/docs/references/config#cssmode) a single class names a whole `css()` call rather than one
declaration. Resolving where it could and joining where it could not would mean the same source behaving one way in
development and another in a minified production build, with no error either way.
