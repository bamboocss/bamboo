...
<Callout type="info">
  This only applies to classes whose property is still visible in the name. With [`hash`](#hashing) or [`cssMode:
  'grouped'`](/docs/references/config#cssmode) every class is an opaque hash, so `cx` can only concatenate – compose
  with `css(a, b)` instead, which merges the style objects before any class name exists.
</Callout>
