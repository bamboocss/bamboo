...
- transform a source file to a `tsx` friendly syntax before it's parsed (`parser:before`) so that Bamboo can
  automatically extract its styles usage
- create your own styles parser (`parser:before`, `parser:after`) using the file's content so that Bamboo could be used
  with any templating language
- alter the generated JS and DTS code (`codegen:prepare`)
- modify the generated CSS (`cssgen:done`), for anything the built-in passes do not cover — rewriting asset URLs,
  prepending a licence banner, running the output through another processor
...
<Callout type="warning">
  ...
  safe in a way a scan of the finished CSS cannot be — they keep the declarations `token()` and `token.var()` resolve to
  at runtime, which nothing in the stylesheet refers to. [`prunePreflight`](/docs/references/config#prunepreflight)
  covers the reset.
</Callout>
