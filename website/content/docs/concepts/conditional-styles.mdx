...
description: Learn how to use conditional and responsive styles in Bamboo.
---
...
Bamboo allows you to write conditional styles, and provides common condition shortcuts to make your life easier. Let's
...
Conditions in Bamboo can be nested, which means you can apply multiple conditions to a single property or another
condition.
...
Every pseudo class in the [Reference](#reference) has a `_` modifier that works like the `_hover` above — `_active`,
`_focus`, `_disabled`, `_first`, `_last`, `_even`, `_odd`, and so on:
...
| --------------------------------------------------- | ---------------------------------------------------------- |
| `_motionReduce` / `_motionSafe`                     | `prefers-reduced-motion: reduce` / `no-preference`         |
| `_osDark` / `_osLight`                              | `prefers-color-scheme: dark` / `light`                     |
| `_lessContrast` / `_moreContrast` / `_highContrast` | `prefers-contrast: less` / `more`, `forced-colors: active` |
| `_portrait` / `_landscape`                          | `orientation: portrait` / `landscape`                      |
...
Container sizes carry the same range set as breakpoints. Alongside `@/sm`, which is open-ended upwards, you get
`@/smOnly` for that size alone, `@/smDown` for everything below it, and `@/smToLg` for a span:
...
Bamboo lets you create your own conditions, so you're not limited to the ones in the default preset. Learn more about
